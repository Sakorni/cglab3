...
class ColorAnalyzer:
    def __init__(self, canvas, cimg, default_color):
    ...
    def get_color(self, x, y):
        ...
        rgb = self.cimg.get(x,y)
        hex_c = color_from_rgb(rgb)
        return [rgb, hex_c]

    def isEmptyPixel(self, x, y, tolerance=None):
        if (x < 0 or x > self.canvas.winfo_reqwidth() - 1):
            return False
        if (y < 0 or y > self.canvas.winfo_reqheight() - 1):
            return False
...
class Drawer:
    def __init__(self, canvas: tkinter.Canvas, cimg, color: tuple):
        self.color = color
        self.canvas = canvas
        self.cimg = cimg
    ...
    def draw_line(self, x1: int, y1: int, x2: int, y2: int, tag: str = None):
        ...
        for i in range(x2 - x1):
            self.cimg.put(self.color[1], (x1+i, y1))
...
class FancyDrawer:
    ...
    def __init__(self, path: str, clicked_x: int, clicked_y: int, canv: tkinter.Canvas, cimg) -> None:
